...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/03 10:25:29 by sgil-moy          #+#    #+#             */
/*   Updated: 2023/10/03 10:48:31 by sgil-moy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	aux = NULL;
	if (ft_find_line(buff[fd]) == 1)
	{
		aux = ft_get_line(buff[fd]);
		buff[fd] = ft_resize_buff(buff[fd]);
		return (aux);
	}
	buff[fd] = get_text(buff[fd], fd);
	if (!buff[fd])
		return (NULL);
	return (result(&buff[fd], aux));
}
